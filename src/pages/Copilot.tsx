...
const CopyButton = ({ text }: { text: string }) => {
  ...
  return (
    <button
      onClick={handleCopy}
      className="absolute top-2 right-2 p-1 rounded bg-primary/10 hover:bg-primary/20 text-primary transition-colors"
      title="Copy"
    >
...
// ── Code block with copy + horizontal scroll ─────────────────────────────────
const CodeBlock = ({ children }: { children: string }) => (
  <div className="relative my-2 group">
    <CopyButton text={children} />
    <pre className="bg-[#0d1117] border border-border rounded-md px-3 py-3 pr-10 overflow-x-auto max-w-full">
  ...
  const handleSend = (text?: string) => {
    ...
    fetch("https://cloud-security-copilot-qd4o.onrender.com/api/chat/", {
      ...
      .then((res) => {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: res.response, tools_used: res.tools_used },
        ]);
      })
      .catch(() => {
        setMessages((prev) => [
          ...prev,
          { role: "assistant", content: mockFallback.content, tools_used: mockFallback.tools },
        ]);
      })
      ...
      <div className="flex-1 overflow-y-auto space-y-4 mb-4 pr-1">
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-3 ${msg.role === "user" ? "justify-end" : ""}`}>
            ...
            <div
              className={`min-w-0 rounded-md px-4 py-3 text-sm leading-relaxed overflow-hidden ${
                msg.role === "user"
                  ? "max-w-[80%] bg-secondary text-foreground"
                  : "w-full bg-card border border-border text-secondary-foreground"
              }`}
            >
              {/* Tool badges */}
              ...
              {msg.role === "user" ? (
                ...
                <ReactMarkdown
                  ...
                  components={{
                    ...
                    h2: ({ children }) => (
                      <h2 className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mt-4 mb-2 first:mt-0">
                        {children}
                      </h2>
                    ),
                    h3: ({ children }) => (
                    ...
                    ol: ({ children }) => (
                      <ol className="space-y-1.5 my-2 ml-1">{children}</ol>
                    ),
                    li: ({ children }) => (
                      <li className="text-sm text-secondary-foreground flex gap-2 items-start list-none">
                        <span className="text-primary shrink-0 leading-5 mt-0.5 font-mono text-xs">▸</span>
                    ...
                    code: ({ children, className }) => {
                      ...
                      return isBlock ? (
                        ...
                        <code className="bg-secondary text-primary px-1.5 py-0.5 rounded text-xs font-mono break-all">
                          {children}
                        </code>
                      );
                    },
                    ...
                    // ── Blockquote ────────────────────────────────────────────
                    blockquote: ({ children }) => (
                      <blockquote className="border-l-2 border-yellow-500/60 bg-yellow-500/5 pl-3 pr-2 py-1.5 my-2 rounded-r-md text-yellow-400/80 text-xs italic">
                        {children}
                      </blockquote>
                    ),
                    ...
                    th: ({ children }) => (
                      <th className="px-3 py-2 text-left font-semibold text-foreground uppercase tracking-wider text-[10px] border-b border-border whitespace-nowrap">
                        {children}
                      </th>
                    ),
                    td: ({ children }) => (
                      <td className="px-3 py-2 text-secondary-foreground align-top">
                        {children}
                      </td>
                    ),
                  }}
        ...
        {isTyping && (
          <div className="flex gap-3">
            ...
            <div className="bg-card border border-border rounded-md px-4 py-3 flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">Querying Elasticsearch + Nova...</span>
            </div>
          </div>
        )}
